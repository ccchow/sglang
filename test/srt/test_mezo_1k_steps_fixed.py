#!/usr/bin/env python3
"""
Fixed 1K step test of MeZO with accuracy objective on RoBERTa SST-2.
Ensures balanced evaluation set and proper gradient computation.
"""
...
def run_1k_step_test():
    ...
    print("=" * 80)
    print("MeZO 1K Step Test - RoBERTa SST-2 with Accuracy Objective (FIXED)")
    print("=" * 80)
    ...
    # Load full dev set and create balanced eval set
    ...
    print(f"  Full dev: {len(full_dev_data)} examples")
    print(f"  Balanced eval: {len(eval_data)} examples")
    
    # Verify balance
    eval_pos = sum(1 for ex in eval_data if ex['label'] == 1)
    eval_neg = sum(1 for ex in eval_data if ex['label'] == 0)
    print(f"  Eval distribution: {eval_pos} positive, {eval_neg} negative")
    ...
    # Sanity check: test on small balanced set
    print("\n  Sanity check on first 10 balanced examples:")
    small_balanced = eval_data[:10]
    small_acc, _ = evaluate(model, tokenizer, small_balanced)
    print(f"    Accuracy: {small_acc:.1%}")
    ...
    if non_zero_grads:
        print(f"\nNon-zero gradients:")
        print(f"  Count: {len(non_zero_grads)}")
        print(f"  Average magnitude: {np.mean(non_zero_grads):.6f}")
        print(f"  Max magnitude: {max(non_zero_grads):.6f}")
    
    # Create plots
    print("\nGenerating plots...")
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('MeZO 1K Steps - RoBERTa SST-2 with Accuracy Objective (Fixed)', fontsize=16)
    ...
    ax3.set_title('Evaluation Accuracy (Balanced Set)')
    ...
    plt.tight_layout()
    plt.savefig('mezo_1k_steps_fixed_results.png', dpi=150)
    print(f"Plots saved to: mezo_1k_steps_fixed_results.png")
    ...
    if final_acc > init_acc + 0.01:  # 1% improvement threshold
        ...
        print("⚠️  MARGINAL: Little to no improvement detected")
        print(f"   Change: {(final_acc-init_acc)*100:.2f}pp - more steps needed")
    else:
        print("❌ DEGRADATION: Accuracy decreased")
    ...
    print(f"- Initial accuracy was {init_acc:.1%} (not 100%!)")
    print("- Balanced evaluation set used (50 pos, 50 neg)")
